        ...
        try:
            ...
            if (menu == 1):
                ...
                for i in range(300, 300 + pid_cycles):
                    ...
                    if i < pid_cycles:
                        thread = PidThread(i, "PID Thread", randrange(10,95))
                    # Set final PidThread to finish last so
                    # that the menu follows completion of all
                    # threads
                    else:
                        thread = PidThread(i, "PID Thread", 100)
                    thread.start()
                thread.join()
